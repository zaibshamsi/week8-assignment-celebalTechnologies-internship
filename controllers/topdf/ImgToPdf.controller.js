...
export const convertImgToPdf = async ( req, res ) => {
    ...
    // convert to jpeg
    const convertRes = await axios.post("https://api.pdf.co/v1/pdf/convert/from/image", {
      name: outputFileName,
      url: uploadedFileUrl      
    }, {
      headers: {
    ...
    writer.on("finish", () => {
     ...
     writer.on("error", (err) => {
      ...
      res.status(500).json({ error: "Failed to write PDF file." });
        ...
        console.error("JPEG to PDF conversion error:", error.response?.data || error.message);
    res.status(500).json({ error: "Failed to convert IMG to PDF." });
    }
} 