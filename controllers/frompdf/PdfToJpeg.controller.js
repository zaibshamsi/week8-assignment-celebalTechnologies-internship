...
export const convertPdfToJpeg = async (req, res) => {
  ...
  try {
    ...
    const convertRes = await axios.post("https://api.pdf.co/v1/pdf/convert/to/jpg", {
    ...
    writer.on("error", (err) => {
      ...
      res.status(500).json({ error: "Failed to write JPG file." });
    });

  } catch (error) {
    console.error("PDF TO JPG conversion error:", error.response?.data || error.message);
